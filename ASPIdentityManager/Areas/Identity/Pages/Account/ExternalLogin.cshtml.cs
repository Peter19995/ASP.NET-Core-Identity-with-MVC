...
namespace ASPIdentityManager.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
        {
            ...
            if (result.Succeeded)
            {
                ...
                var user = _db.ApplicationUser.FirstOrDefault(u => u.Email.ToLower() == info.Principal.FindFirstValue(ClaimTypes.Email).ToLower());
                var claim = await _userManager.GetClaimsAsync(user);

                if (claim.Count > 0)
                {
                    var claimToRemove = claim.FirstOrDefault(u => u.Type == "FirstName");
